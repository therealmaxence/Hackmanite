...
_EXTRACTABLE = {".pdf", ".docx", ".xlsx", ".pptx", ".txt", ".html", ".htm", ".xml", ".md", ".csv"}
...
def parse_eml_file(file_path: Path) -> List[dict]:
    try:
        ...
        raw_id = msg.get("Message-ID") or msg.get("Message-Id")
        if raw_id:
            message_id = raw_id.strip()
        else:
            message_id = f"<{hashlib.sha1(f'{date_raw}-{from_addr}-{subject}'.encode('utf-8', errors='replace')).hexdigest()}@datalake.local>"
        ...
        for part in msg.walk():
            ...
            if disposition == "attachment" or filename:
                ...
                if payload and filename:
                    ...
                    if suffix in _EXTRACTABLE:
                        try:
                            ...
                            prepared = prepare_input(str(tmp_path), MIME_MAP.get(suffix, "text/plain"))
                ...
                attachments.append({"filename": filename or "unnamed_attachment", "size": size, "entities": entities_list})
                continue
            ...
            if ct in {"text/plain", "text/html"}:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                if payload:
                    (body_parts if ct == "text/plain" else html_parts).append(payload.strip())

        if body_parts:
            body_text = "\n\n".join(body_parts)
        elif html_parts:
            body_text = "\n\n".join(_clean_html(h) for h in html_parts)
        else:
            body_text = ""

        return extract_nested_emails({
            "message_id": message_id, "in_reply_to": in_reply_to, "references": references,
            "subject": subject, "from_address": from_addr, "to_address": to_addr,
            "cc_address": cc_addr, "date": parse_date_to_iso(date_raw),
            "body": body_text.strip(), "attachments": attachments,
        })
    except Exception as e:
...
def _list_pst_eml_files(file_path: Path, temp_path: Path) -> List[Path]:
    if not shutil.which("readpst"):
        logger.warning("readpst not found in PATH; skipping PST extraction.")
        return []
    ...
    if result.returncode != 0:
        ...
        if not any(temp_path.glob("**/*")):
            return []
    return sorted((p for p in temp_path.glob("**/*") if p.is_file()), key=lambda p: p.as_posix())
...
def parse_pst_emails(file_path: Path) -> List[dict]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="pst_extract_") as tmp:
            for f in _list_pst_eml_files(file_path, Path(tmp)):
                if parsed := parse_eml_file(f):

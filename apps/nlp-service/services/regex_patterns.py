...
ENTITY_REGEX_PATTERNS: list[tuple[EntityType, re.Pattern[str]]] = [
    ...
    (EntityType.ADDRESS,        re.compile(
        ...
        r"|"
        r"(?:улица|ул\.|проспект|пр-т|пр\.|переулок|пер\.|бульвар|б-р|набережная|наб\.|площадь|пл\.|шоссе|ш\.|аллея|проезд|тупик)"
        r"\s+[А-ЯЁа-яё][А-ЯЁа-яёA-Za-z\s\-]{1,40},?\s*\d{1,5}"
        r"|"
        r"[А-ЯЁа-яё][А-ЯЁа-яё\s\-]{2,40}"
        r"\s+(?:улица|ул\.|проспект|пр-т|переулок|пер\.|бульвар|б-р|набережная|наб\.|площадь|пл\.)"
        r",?\s*\d{1,5}",

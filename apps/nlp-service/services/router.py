...
REGEX_ONLY_MIMES = {
    ...
    "application/xml", "text/xml", "text/x-log", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
...
DOCUMENT_MIMES = {
    ...
    "application/rtf", "text/plain", "text/markdown", "message/rfc822", "application/vnd.ms-outlook",
    "application/vnd.ms-outlook-pst", "application/x-outlook-pst"
}
...
def decide_routing(file_path: str, mime_type: str) -> ExtractionTier:
    ...
    if mime_type in REGEX_ONLY_MIMES or ext in REGEX_ONLY_EXTENSIONS:
        logger.info(f"Router → TIER0_STRUCTURED [{path.name}] (structured)")
        return ExtractionTier.TIER0_STRUCTURED

    if mime_type == "application/pdf":
        try:
            reader = PdfReader(file_path)
            if reader.pages and len((reader.pages[0].extract_text() or "").strip()) > 0:
                logger.info(f"Router → TIER1_DOCUMENT [{path.name}] (text pdf)")
                return ExtractionTier.TIER1_DOCUMENT
            logger.info(f"Router → TIER2_OCR [{path.name}] (scanned pdf)")
            return ExtractionTier.TIER2_OCR
        except Exception as exc:
            logger.warning(f"Router PDF check failed for {path.name}: {exc}. Defaulting to OCR.")
            return ExtractionTier.TIER2_OCR

    if mime_type in DOCUMENT_MIMES or ext in DOCUMENT_EXTENSIONS:
        logger.info(f"Router → TIER1_DOCUMENT [{path.name}] (semi-structured)")
        return ExtractionTier.TIER1_DOCUMENT
            
    if mime_type in IMAGE_MIMES:
        logger.info(f"Router → TIER2_OCR [{path.name}] (image)")
        return ExtractionTier.TIER2_OCR

    logger.info(f"Router → TIER0_STRUCTURED [{path.name}] (default)")
    return ExtractionTier.TIER0_STRUCTURED

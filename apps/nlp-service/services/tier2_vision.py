...
def extract(file_path: str, mime_type: str) -> str:
    ...
    if input_type == "images":
        page_count = len(prepared["data"])
        logger.info("Tier2: running OCR on %d pages for %s", page_count, filename)

        ocr_results = []
        for page_num, img_data in enumerate(prepared["data"], start=1):
            page_text = image_to_text(img_data)
            if page_text:
                ocr_results.append(page_text)
            else:
                logger.warning("Tier2: OCR returned empty result for page %d of %s", page_num, filename)

...
export async function POST(
  ...
  try {
    ...
    if (isRedisAvailable && bullQueue) {
      ...
      logger.info(`[Pipeline] Dispatched run ${run.id} to BullMQ.`);
    } else {
      Promise.resolve().then(async () => {
        try {
          await executePipeline(run.id, sessionId);
        } catch (err) {
          logger.error(`In-memory execution failed for run ${run.id}:`, { err });
        }
      });
      logger.info(`[Pipeline] Dispatched run ${run.id} in-memory (async).`);
    }

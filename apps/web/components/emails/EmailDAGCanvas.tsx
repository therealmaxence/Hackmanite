...
export default function EmailDAGCanvas({ elements, layoutType, onNodeSelect, onBackgroundTap, selectedEmailId }: EmailDAGCanvasProps) {
  ...
  const toggleFullscreen = () => {
    ...
    if (!document.fullscreenElement) {
      wrapperRef.current.requestFullscreen?.().then(() => setIsFullscreen(true)).catch(() => setIsFullscreen(true));
    } else {
  ...
  useEffect(() => {
    ...
    if (cy) {
      setTimeout(() => { cy.resize(); cy.fit(undefined, 40); }, 150);
    }
  }, [isFullscreen, cyRef]);

  return (
    <div ref={wrapperRef} style={{ flex: 1, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', ...(isFullscreen ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100vh', zIndex: 9999 } : {}) }}>
      ...
      <button
        onClick={toggleFullscreen} title={isFullscreen ? t('graph.canvas.exit_fullscreen') : t('graph.canvas.fullscreen')}
        style={{ position: 'absolute', top: '1rem', right: '1rem', zIndex: 50, width: '40px', height: '40px', borderRadius: 'var(--radius)', background: 'var(--color-surface-raised) var(--noise-bg)', border: 'none', color: 'var(--color-text)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.5)', transition: 'all 0.15s ease' }}
        onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--color-primary-hover)'; }}
        onMouseLeave={(e) => { e.currentTarget.style.color = 'var(--color-text)'; }}
      >
        {isFullscreen ? (
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 14h6v6M20 10h-6V4M14 10l7-7M10 14l-7 7" /></svg>
        ) : (

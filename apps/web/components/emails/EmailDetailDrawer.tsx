...
export default function EmailDetailDrawer({ email, senderColors, rawEdges, rawEmails, onSelectEmail, focusedThreadRootId, onClose, onFocusThread, onResetFocus, onDelete }: Props) {
  ...
  const tabStyle = (tab: 'content' | 'timeline') => ({
    ...
    fontWeight: activeTab === tab ? 600 : 400, fontSize: '0.8125rem', cursor: 'pointer',
    transition: 'all 0.15s ease', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
  });

  return (
    <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 420, zIndex: 60, background: 'var(--color-surface)', display: 'flex', flexDirection: 'column', animation: 'slideIn 0.25s cubic-bezier(0.16,1,0.3,1) forwards' }}>
      <style>{`@keyframes slideIn { from { transform: translateX(100%); } to { transform: translateX(0); } }`}</style>
      <div style={{ padding: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontSize: '0.7rem', background: 'color-mix(in srgb, var(--color-primary) 15%, var(--color-surface-raised))', borderRadius: 'var(--radius-sm)', padding: '2px 8px', color: 'var(--color-primary)', fontWeight: 500, textTransform: 'uppercase', fontFamily: 'var(--font-mono)' }}>{t('emails.drawer.node_title')}</span>
      ...
      <div style={{ display: 'flex', background: 'var(--color-surface-input)' }}>
      ...
      <div style={{ flex: 1, padding: '1.25rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        {activeTab === 'content' ? (
          <EmailContentTab email={email} fromColor={fromColor} isFocused={focusedThreadRootId === email.messageId} onFocusThread={() => onFocusThread(threadRootId)} onResetFocus={onResetFocus} onDelete={onDelete} />
        ) : (
          <EmailThreadTimeline currentEmail={email} threadEmails={threadEmails} threadRootId={threadRootId} senderColors={senderColors} onSelectEmail={onSelectEmail} />
        )}
      </div>
    </div>
  );

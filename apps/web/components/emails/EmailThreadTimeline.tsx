...
function formatSenderDisplay(from: string) {
  return from.includes('<') ? from.substring(0, from.indexOf('<')).trim() || from : from.split('@')[0];
}

function TimelineItem({ item, isActive, isFirst, isLast, threadRootId, senderColors, onSelect }: { item: Record<string, unknown>; isActive: boolean; isFirst: boolean; isLast: boolean; threadRootId: string; senderColors: Record<string, string>; onSelect: () => void }) {
  const { t } = useTranslation();
  const msgId = item.messageId as string, from = (item.from as string) || '';
  ...
  const dateStr = item.date ? new Date(item.date as string).toLocaleDateString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : t('emails.timeline.no_date');
  ...
  return (
    <div style={{ display: 'flex', gap: '0.875rem', position: 'relative' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 20 }}>
        ...
        <div style={{ width: 18, height: 18, borderRadius: '50%', border: `2px solid ${fromColor}`, display: 'flex', alignItems: 'center', justifyContent: 'center', background: isActive ? fromColor : 'var(--bg-base)', zIndex: 2 }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem' }}>
          <span style={{ fontSize: '0.75rem', fontWeight: 600, color: fromColor, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 150 }}>{formatSenderDisplay(from)}</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
          ...
          {(item.attachments as unknown[])?.length > 0 && <span style={{ fontSize: '0.625rem', background: 'var(--color-surface-input)', color: 'var(--color-text)', padding: '1px 5px', borderRadius: 'var(--radius-sm)' }}>{(item.attachments as unknown[]).length}</span>}
        </div>
        {bodySnippet && <div style={{ fontSize: '0.6875rem', color: 'var(--color-text-muted)', fontFamily: 'var(--font-mono)', lineHeight: 1.3, marginTop: 2, background: 'var(--color-surface-input)', padding: '4px 6px', borderRadius: 'var(--radius)', wordBreak: 'break-all' }}>{bodySnippet}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      {threadEmails.map((item, idx) => (
        <TimelineItem key={item.messageId as string} item={item} isActive={item.messageId === currentEmail.messageId} isFirst={idx === 0} isLast={idx === threadEmails.length - 1} threadRootId={threadRootId} senderColors={senderColors} onSelect={() => onSelectEmail(item)} />
      ))}
    </div>
  );
}

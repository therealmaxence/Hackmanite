...
export default function SessionClient() {
  const { sessionId, sessions, isLoadingSessions, handleSwitchSession, handleDeleteSession, handleDeleteAllSessions, handleStartNewSession } = useSessionManager();
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg-base)' }}>
      ...
      <main style={{ flex: 1, overflowY: 'auto', padding: 'clamp(2rem, 5vw, 4rem) clamp(1rem, 5vw, 3rem)' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '3rem' }}>
          ...
          <LocalSessionManager isLoadingSessions={isLoadingSessions} sessions={sessions} sessionId={sessionId} handleStartNewSession={handleStartNewSession} handleSwitchSession={handleSwitchSession} handleDeleteSession={handleDeleteSession} handleDeleteAllSessions={handleDeleteAllSessions} />

...
export default function BridgeEntitiesTable({ bridgeEntities }: { bridgeEntities: BridgeEntity[] }) {
  const { t } = useTranslation();
  return (
    <div className="signature-card flex flex-col justify-between" style={{ padding: '2.5rem', gap: '2.5rem' }}>
      <h3 className="text-sm font-semibold text-white/70">{t('stats.bridges.title')}</h3>
            ...
            {bridgeEntities.map((entity, i) => (
              <div key={entity.id} className="flex items-center justify-between border-b border-white/[0.04] pb-3 last:border-0 last:pb-0">
                ...
                <div className="text-right shrink-0">
                  <span className="font-mono text-xs text-accent bg-accent/5 border border-accent/15 px-2 py-0.5 rounded-sm">{entity.score.toFixed(3)}</span>
                </div>
              </div>
            ))}
          </div>

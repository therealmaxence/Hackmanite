...
export default function SimilarityHeatmap({
  ...
  return (
    <div
      className="lg:col-span-2 signature-card flex flex-col justify-between"
      style={{
        padding: '2.5rem',
        gap: '2.5rem',
      }}
    >
      <h3 className="text-sm font-semibold text-white/70">
        Document similarity matrix (Jaccard overlap)
      </h3>
      
      {similarityData.files.length === 0 ? (
        <div className="h-[300px] flex items-center justify-center">
          <p className="text-xs font-mono text-white/30 uppercase tracking-widest">No document pairs found</p>
        </div>
      ) : (
        <>
          <div className="overflow-x-auto custom-scrollbar flex-1 w-full">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  <th className="pb-3 pr-4 text-[10px] tracking-wider text-white/35 text-left font-mono min-w-[140px] border-b border-white/10">
                    Document
                  </th>
                  {similarityData.files.map((file) => (
                    <th
                      key={file.id}
                      className="pb-3 px-3 text-[10px] tracking-wider text-white/35 font-mono text-center max-w-[120px] truncate border-b border-white/10"
                      title={file.name}
                    >
              ...
              <tbody>
                {similarityData.files.map((fileA, i) => (
                  <tr key={fileA.id} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.01] transition-colors">
                    <td
                      className="py-4 pr-4 text-xs font-semibold text-white/70 truncate max-w-[140px] font-mono"
                      title={fileA.name}
                    >
                      {fileA.name}
                    </td>
                    {similarityData.files.map((fileB, j) => {
                      ...
                      const isSelected = selectedCell && (
                        (selectedCell.fileA.id === fileA.id && selectedCell.fileB.id === fileB.id) ||
                        (selectedCell.fileA.id === fileB.id && selectedCell.fileB.id === fileA.id)
                      );
                      return (
                        <td
                          key={fileB.id}
          ...
          {selectedCell ? (
            <div
              className="p-4 rounded-sm flex flex-col md:flex-row md:items-center justify-between gap-4 animate-fadeIn"
              style={{
            ...
            >
              <div className="space-y-1.5">
                <div className="text-[10px] tracking-widest text-white/35 font-mono">
                  Selected Document Connection
                </div>
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  ...
                  <span className="text-white/35 font-mono text-[10px] tracking-wider px-1">and</span>
              ...
              <div className="flex items-center gap-6 shrink-0 justify-between md:justify-end">
                <div className="text-right">
                  <div className="text-[10px] tracking-widest text-white/35 font-mono">Jaccard Overlap</div>
            ...
            <div className="p-4 rounded-sm border border-dashed border-white/10 text-center bg-white/[0.01]">
              <p className="text-[10px] font-mono text-white/30 uppercase tracking-widest leading-relaxed">
                Click any cell in the similarity matrix to inspect document relationships
              </p>
            </div>
          )}
        </>

...
export default function BridgeSignalsTable({ signals }: Props) {
  const { t } = useTranslation();
  return (
    <div className="signature-card flex flex-col justify-between" style={{ padding: '2.5rem', gap: '2.5rem' }}>
      <h3 className="text-sm font-semibold text-white/70">{t('weak_signals.bridges.title')}</h3>
      {signals.length === 0 ? (
        <div className="h-[300px] flex items-center justify-center">
          <p className="text-xs font-mono text-white/30 uppercase tracking-widest">{t('weak_signals.bridges.no_data')}</p>
        </div>
      ) : (
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      <span className="text-[9px] tracking-widest text-white/30 font-mono">
                        {t(`entity.${entity.type}`)} &middot; {entity.totalCount} {t('statusbar.files').slice(0, -1)} count{entity.totalCount > 1 ? 's' : ''}
                      </span>
                    </div>
                  </div>
                </div>

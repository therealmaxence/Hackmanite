...
export default function EmergingSignalsTable({ signals }: Props) {
  const { t } = useTranslation();
  return (
    <div className="signature-card flex flex-col justify-between" style={{ padding: '2.5rem', gap: '2.5rem' }}>
      <h3 className="text-sm font-semibold text-white/70">{t('weak_signals.emerging.title')}</h3>
      {signals.length === 0 ? (
        <div className="h-[300px] flex items-center justify-center">
          <p className="text-xs font-mono text-white/30 uppercase tracking-widest text-center" style={{ maxWidth: '80%' }}>
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      <span className="text-[9px] tracking-widest text-white/30 font-mono">
                        {t(`entity.${entity.type}`)} &middot; {t('weak_signals.emerging.peak_count', { count: entity.totalCount, plural: entity.totalCount !== 1 ? 's' : '' })}
                      </span>
                    </div>
                  </div>
                </div>

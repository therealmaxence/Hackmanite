...
5. Pistes d'Approfondissement (thèmes ou entités qui méritent une investigation complémentaire)`,
      objectivityGuidelines: `=== DIRECTIVES D'OBJECTIVITÉ ===
...
5. Areas for Further Investigation (themes or entities that warrant deeper exploration)`,
      objectivityGuidelines: `=== OBJECTIVITY GUIDELINES ===
...
- Adapt the tone to what the data actually shows — the corpus may be academic, professional, journalistic, or any other nature.`,
      audience: 'Intended audience: general professionals (managers, researchers, analysts). Avoid excessive technical jargon; use clear and precise language.'
    }
  }[langKey];

  const focusDescMap: Record<string, Record<string, string>> = {
    fr: {
      actors: 'Focus sur les Acteurs & Identifiants Clés (mettre en évidence les entités PERSON, ORG, EMAIL, IP_ADDRESS et les nœuds de communication/infrastructure les plus saillants).',
      networks: 'Focus sur les Liaisons & Clusters (mettre en évidence les nœuds structurels centraux, les co-occurrences fréquentes et les groupes topologiques).',
      timeline: "Focus sur la Chronologie (synthétiser les dates, les périodes d'activité et les séquences temporelles).",
      general: 'Analyse Générale (vue d\'ensemble complète des entités, relations et tendances du corpus).',
    },
    en: {
      actors: 'Key Actors & Identifiers Focus (highlight PERSON, ORG, EMAIL, IP_ADDRESS, and the most salient communication/infrastructure nodes).',
      networks: 'Linkage & Cluster Focus (highlight central structural nodes, frequent co-occurrences, and topological groups).',
      timeline: 'Timeline Focus (synthesize dates, activity periods, and temporal sequences).',
      general: 'General Analysis (comprehensive overview of entities, relationships, and trends across the corpus).',
    }
  };

  const focusDesc = focusDescMap[langKey][focusType] || focusDescMap[langKey].general;

  const weakSignalsText = selectedWeakSignals.length > 0
    ? `${config.weakSignalNote}\n${selectedWeakSignals.map((ws: any) => `- ${ws.label} (${ws.type}): Score ${ws.score.toFixed(3)} [Method: ${ws.methodology}]`).join('\n')}`
    : config.weakSignalsNone;
